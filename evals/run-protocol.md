...
- whether the skill is withheld (`RED`) or loaded (`GREEN`);
...
2. For `RED`, do not expose the new skill. For `GREEN`, instruct the agent to read `skills/learning-ai-architectures-from-source/SKILL.md` completely and then follow its progressive-disclosure routing.
